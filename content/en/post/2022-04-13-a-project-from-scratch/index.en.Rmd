...
The output of this workflow can be found in [its GitHub repository](https://github.com/montesmariana/doenLaten), but the idea is to give you the instructions to start it _from scratch_. Let's go!
...
The simple odds, which are more easily interpretable, can be computed with `exp()`. In this case, the log odds of the Intercept are `r sprintf("%.3f", m$coefficients[['(Intercept)']])`, and the simple log odds are therefore `r sprintf("%.3f", exp(m$coefficients[['(Intercept)']]))`, which is equal to 20/3, the respective frequencies of *doen* and *laten* when all predictors are at their reference level. The interpretation is that, when all predictors are at their reference level, *doen* is `r sprintf("%.3f", exp(m$coefficients[['(Intercept)']]))` times more likely than *laten*.

```{r}
exp(m$coefficients[['(Intercept)']])
```

When it comes to the predictors, the `Estimate` refers to the log odds of *doen* for a given value of the variable (e.g. *Tr* for `EPTrans`) compared to the reference value (*Intr* for `EPTrans`). Negative log odds indicate that *doen* is less likely than the reference response value *laten*; the simple odds of *doen* against *laten* when the Effected Predicate is Transitive, all other variables being controlled for, are then `r sprintf("%.3f", exp(m$coefficients[['EPTransTr']]))`, i.e. *doen* is about one fifth as likely as *laten*.
...
```{r, startRmd, echo = FALSE, fig.cap = '(ref:startRmd)', fig.alt="Two screenshots of the steps to create an R Markdown file, slightly overlapping. The first shows a dropdown menu with the option to create the file. The second shows a dialog window with that lets you name your file and select an initialization configuration.", out.width = '40%'}
...
The R Markdown raw file is shown below; the HTML output is shown in \@ref(fig:report1)`.
...
```{r, visnet1, echo = FALSE, fig.cap = '(ref:visnet1)', fig.alt="Linear network of blue circles and triangles representing the current pipeline.", out.width = '70%'}
...
```{r, visnet2, echo = FALSE, fig.cap = '(ref:visnet2)', fig.alt="Linear network of blue circles and triangles representing the current pipeline.", out.width = '70%'}
...
```{r, visnet4, echo = FALSE, fig.cap = '(ref:visnet4)', fig.alt="Linear network of blue and black circles and triangles representing the current pipeline.", out.width = '70%'}
